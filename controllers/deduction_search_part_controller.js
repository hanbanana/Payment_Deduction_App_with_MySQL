...
router.get("/deduction_pages/add_part_purchase_deduction/:input_truck_payment_truck_no", function (req, res) {
    connection.query("SELECT * FROM bc_deduction_db.input_part_purchase_db where id in (SELECT MAX(id) FROM bc_deduction_db.input_part_purchase_db where input_part_purchase_truck_no = ?)", [req.params.input_truck_payment_truck_no], function (err, partPurchaseData) {
        connection.query("SELECT * FROM information_owner_db;", function (err, ownerData) {
            connection.query("SELECT * FROM information_driver_db;", function (err, driverData) {
                connection.query("SELECT * FROM information_truck_db;", function (err, truckData) {
                    if (err) {
                        ...
                        res.render("deduction_pages/add_part_purchase_deduction", {partPurchaseData, ownerData, driverData, truckData });
                    };
                });
            });
        });
...
router.post("/input_part_purchase_list", function (req, res) {
    connection.query("INSERT INTO input_part_purchase_db (input_part_purchase_status, input_part_purchase_type, input_part_purchase_truck_no, input_part_purchase_owner_id, input_part_purchase_driver_id, input_part_purchase_invoice_no, input_part_purchase_amount, input_part_purchase_pay_week, input_part_purchase_date, input_part_purchase_add_pay, input_part_purchase_cr, input_part_purchase_invoice, input_part_purchase_desc, input_part_purchase_paid_amount, input_part_purchase_this_time_payment, input_part_purchase_paid_date, input_part_purchase_balance_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [req.body.input_part_purchase_status, req.body.input_part_purchase_type, req.body.input_part_purchase_truck_no, req.body.input_part_purchase_owner_id, req.body.input_part_purchase_driver_id, req.body.input_part_purchase_invoice_no, req.body.input_part_purchase_amount, req.body.input_part_purchase_pay_week, req.body.input_part_purchase_date, req.body.input_part_purchase_add_pay, req.body.input_part_purchase_cr, req.body.input_part_purchase_invoice, req.body.input_part_purchase_desc, req.body.input_part_purchase_paid_amount, req.body.input_part_purchase_this_time_payment, req.body.input_part_purchase_paid_date, req.body.input_part_purchase_balance_amount], function (err, result) {
            if (err) {
                return res.status(500).end();
            }
